    ...
    if st.button("Add Race"):
        if not (race_id and race_name and track_name): # check if user entered all the required info
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO Race VALUES (%s, %s, %s, %s, %s)", # The SQL query to add a new race into the Race table
    ...
    if st.button("Delete Owner"):
        if not owner_id: # check if user entered an owner ID or not
            ...
            try:
                conn = get_connection()
                cursor = conn.cursor()
                cursor.callproc("delete_owner", [owner_id]) # calling the stored procedure | When the admin deletes a owner
                conn.commit()                               # all horse related info will be stored in and old_info table, written as a trigger after horse deletion
